...
# Para ejecutar este Script pulsa CTRL+R (Windows) 
...
# Cargar los paquetes instalados #hecho
library(UsingR)
...
sqrt(10) #esto es la raiz cuadrada
...
# Nombres de variables: pueden incluir puntos, "_" pero NUNCA "-". NUNCA empiezan por números. en los nombres puedo usar puntos.
...
# Diferentes maneras de crearlos y operar con ellos. R me deja sobreescribir sobre las variables anteriores, es decir, si x antes era 3 y yo ahora quiero que sea un string puedo hacerlo.

# Crearlos con la función c(): 
x<-c(2,0,0,4)
y<-c(1,9,9,9)
u=c(1,4,2,4) #este lo he añadido yo a ver 
...
rep(1,10) #que repita 1 10 veces

seq(2,6)#que nos de una lista del 2 al 6 incluyendo el fin = 6. 

seq(4,20,by=4)#podemos añadir la lista de 4 en 4.
...
y[y<9]=2 # reasigna solo si cumple la condicion. que todos los valores de y que sean menores de 9 sean 2 ahora
y
...
bp=read.table("bp.txt",header=T) # así leemos el archivo. entre parentesis el nombre edl archivo y luego header true
bp
...
head(iris)       # dataset integrado en R. con head me salen las 5 primeras, si pusiera "tail" serian las 5 ultimas
str(iris) 
summary(iris) #nos da caracteristicas de la tabla

iris$Species #que me saque los valores de la columna de species
...
##### 7.- VECTORES / LISTAS AVANZADOS esto nos da mas igual ya que no vamos a verlo
...
genesets<-vector("list",3)#tengo 3 posiciones, siendo cada una de ellas mis otras listas
genesets[[1]]<-geneset1
genesets[[2]]<-geneset2
genesets[[3]]<-geneset3
genesets
...
names(genesets)=paste0("set",1:3) #paste0 me da un nombre comun 
genesets
...
genesets[[1]][1]
unlist(genesets[1])[1]
length(genesets)#nos da el numero de elementos
length(genesets[[2]])
...
for(j in 1:3){
lista_vacia[[j]] <- c(1,2,3*j) # j es un indicador de posicion (tambien se usa la y)
}
lista_vacia
...
while(valor < umbral) {print("Todavía no.")valor <- valor + 1}
...
#aplican la media a una columna entera de una tabla
...
ggplot(iris, aes(x = Petal.Length, y = Petal.Width, color = Species)) +
  geom_point(size = 3) + labs(title = "Relación entre largo y ancho de pétalos") +
  theme_minimal()
